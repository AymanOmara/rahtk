...
{
    public class ProductRepository(RahtkContext context, IFileService fileService, UserManager<RahtkUser> userManager) : IProductRepository
    {
        public async Task<ICollection<ProductEntity>> GetAllProducts()
        {
            var products = await context.Products.AsNoTracking().Where(pr=>!pr.Deleted).ToListAsync();
        ...
        {
            var productPath = await fileService.SaveFileAsync(file);
        ...
        public async Task<ProductEntity> AddToFavorite(string email, int productId)
        {
            ...
            var product = await context.Products.Include(pro => pro.FavoriteProductUsers).Include(cat=>cat.Category).FirstOrDefaultAsync(pro => pro.Id == productId);
        ...
        {
            var user = await userManager.FindByEmailAsync(email);
            var favorite = await context.FavoriteProductUser.Include(p => p.Product).ThenInclude(cat=>cat.Category).Include(p => p.User).FirstOrDefaultAsync(prod => prod.UserId == user.Id && prod.ProductId == productId);
        ...
        public async Task<ProductEntity> GetProductDetails(string email, int productId)
        {
            ...
            var product = await context.Products.AsNoTracking().Include(pro=> pro.Category).FirstOrDefaultAsync(prod => productId == prod.Id);
